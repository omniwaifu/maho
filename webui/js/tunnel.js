...
document.addEventListener('alpine:init', () => {
    Alpine.data('tunnelSettings', () => ({
        ...
        async checkTunnelStatus() {
            try {
                const response = await fetch('/api/v1/tunnel_proxy', {
                    method: 'POST',
                    headers: {
                ...
                if (data.success && data.tunnel_url) {
                    ...
                    if (storedTunnelUrl) {
                        ...
                        const verifyResponse = await fetch('/api/v1/tunnel_proxy', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
        ...
        async refreshLink() {
            ...
            if (confirm("Are you sure you want to generate a new tunnel URL? The old URL will no longer work.")) {
                ...
                try {
                    ...
                    const stopResponse = await fetch('/api/v1/tunnel_proxy', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
        ...
        async generateLink() {
            ...
            try {
                const authCheckResponse = await fetch('/api/v1/settings_get');
                ...
                if (!hasAuth) {
                    const proceed = confirm(
                        "WARNING: No authentication is configured for your Maho instance.\n\n" +
                        "Creating a public tunnel without authentication means anyone with the URL " +
                        "can access your Maho instance.\n\n" +
            ...
            try {
                ...
                if (data.success && data.tunnel_url) {
                    ...
                    try {
                        const statusResponse = await fetch('/api/v1/tunnel_proxy', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
